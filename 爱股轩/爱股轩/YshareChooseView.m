...
-(void)initView{
    UILabel *titleLb = [[UILabel alloc]initWithFrame:CGRectMake((__kWidth-AutoWidth(100))/2, 0, AutoWidth(100), AutoWidth(13))];
    [self addSubview:titleLb];
    titleLb.textColor = LH_RGBCOLOR(159, 159, 159);
    titleLb.textAlignment = NSTextAlignmentCenter;
    titleLb.font = MFont(AutoWidth(12));
    titleLb.text = @"社交账号一键登录";

    NSArray *imageArr = @[@"qq_icon_login",@"wx_icon_login",@"wb_icon_login"];
    for (int i=0; i<3; i++) {
        UIButton *shareBtn = [[UIButton alloc]initWithFrame:CGRectMake(AutoWidth(30)+i*(__kWidth-AutoWidth(120))/2, AutoWidth(28), AutoWidth(60), AutoWidth(60))];
        [self addSubview:shareBtn];
        shareBtn.layer.cornerRadius = AutoWidth(30);
...
-(void)drawRect:(CGRect)rect{
    ...
    CGContextMoveToPoint(ctx, __kWidth/2+AutoWidth(50), AutoWidth(7));  //起点坐标
    CGContextAddLineToPoint(ctx, __kWidth-AutoWidth(30), AutoWidth(7));   //终点坐标
    ...
    CGContextMoveToPoint(ctx1, AutoWidth(30), AutoWidth(7));  //起点坐标
    CGContextAddLineToPoint(ctx1, (__kWidth-AutoWidth(100))/2, AutoWidth(7));   //终点坐标
